...
{ pkgs, config, lib, ... }: let
  ...
  generatedPackageIndex = let
    p = lib.flatten [cfg.packageIndex];
  in lib.foldl' (acc: x: let
    value =
  ...
  in acc // value // { packages = (acc.packages or []) ++ (value.packages or []); }) {} p;
  generatedLibraryIndex = let
    l = lib.flatten [cfg.libraryIndex];
  in lib.foldl' (acc: x: let
    value =
      if lib.isAttrs x then
        x
  ...
  in acc // value // { libraries = (acc.libraries or []) ++ (value.libraries or []); }) {} l;
  indexType = with lib.types;
    oneOf [ pathInStore (listOf (oneOf [ (attrsOf anything) pathInStore ])) ];
in {
  options.arduino-cli = {
    ...
    packageIndex = lib.mkOption {
      type = indexType;
      default = "${inputs.arduino-index}/index/package_index.json";
    };
    libraryIndex = lib.mkOption {
      type = indexType;
      default = "${inputs.arduino-index}/index/library_index.json";
    };
  };
